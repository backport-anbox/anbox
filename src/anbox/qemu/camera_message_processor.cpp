...
namespace anbox {
namespace qemu {
class CameraMessageProcessor::ConnectionParameters {
 public:
  /* Client name.
...
CameraMessageProcessor::~CameraMessageProcessor() {
  if (connection_data_) {
    ERROR("Camera client for device '%s' on input channel %d is now closed",
      connection_data_->device_name.c_str(), connection_data_->inp_channel);
  }
}
...
void CameraMessageProcessor::handle_command(const std::string &command) {
  ...
  ERROR("%s: query '%s'", __FUNCTION__, query.query_name.c_str());
...
void CameraMessageProcessor::list() {
  ...
  ERROR("Camera service descriptor at %p", csd.get());
  ...
  ERROR("Replied: %s", reply.c_str());
  reply_ok(reply);
}

void CameraMessageProcessor::connect(const std::string &params) {
  ERROR("%s:line:[%d]", __FUNCTION__,__LINE__);
  UNUSED(params);
  if (!connection_data_) {
...
void CameraMessageProcessor::disconnect(const std::string &param) {
  ...
  ERROR("Camera device '%s' is now disconnected", connection_data_->device_name.c_str());
...
void CameraMessageProcessor::start(const std::string &param) {
  ...
  */
  ...
  ERROR("Camera '%s' is now started for %.4s[%dx%d]",
...
void CameraMessageProcessor::stop(const std::string &param) {
  ...
  ERROR("Camera device '%s' is now stopped.", connection_data_->device_name.c_str());
  reply_ok();
}

void CameraMessageProcessor::frame(const std::string &param) {

  ERROR("%s: query frame:'%s'", __FUNCTION__, param.c_str());
  size_t video_size = 0;
  ...
  if (tmp) {
    if (sscanf(tmp->c_str(), "%g,%g,%g", &r_scale, &g_scale, &b_scale) != 3) {
      ERROR("Invalid value '%s' for parameter 'whiteb'", tmp->c_str());
